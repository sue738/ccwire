#!/usr/bin/env node
/**
 * ccwire 🔌 — see what your Claude Code session actually sends to the API.
 *
 ...
 *   ccwire 657f           # a session by id prefix, any project
 *   ccwire --turns 10     # per-request token growth
 *   ccwire --diff         # what the last request added
 *   ccwire proxy          # live: the bytes a transcript can't show you
 *   ccwire proxy-report   # summarize what the proxy has logged
 */
'use strict';
...
// 出力言語: 既定は英語、ロケールが日本語のときだけ日本語(CC_WIRE_LANGで明示指定も可)
const JA = /^ja/i.test(process.env.CC_WIRE_LANG || process.env.LC_ALL || process.env.LC_MESSAGES || process.env.LANG || '');
const L = (en, ja) => (JA ? ja : en);

const HELP = `ccwire — what your Claude Code session actually sends to the API

Usage: ccwire [session-id-prefix] [options]
...
function printOverview(info, a) {
  ...
  console.log(`ccwire — ${info.session.slice(0, 8)} · ${T.projectName(info.projectDir)} · ${a.requests.length} ${L('requests', 'リクエスト')}${last && last.model ? ' · ' + last.model : ''}`);
...
function printCache(daily) {
  console.log('=== ccwire --daily --cache ===');
  if (!daily.length) { console.log(L('(no requests recorded)', '(記録されたリクエストなし)')); return; }
...
function printBreakdown(b) {
  console.log(`=== ccwire --daily --breakdown (${b.sessions} ${L('sessions', 'セッション')}) ===`);
...
function printBaseline(daily) {
  console.log('=== ccwire --daily --baseline ===');
 ...
 * (cctoolstatの--dailyと同じ、エントリ単位でsinceMsを切る境界処理)。
 */
...
function printInterrupt(daily) {
  console.log('=== ccwire --daily --interrupt ===');
...
main().catch((e) => { console.error('ccwire:', e.message); process.exit(1); });
