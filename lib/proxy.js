 ...
 *   ccsendstats proxy-report                               # what happened
 *
 ...
 *
 * Kept separate from ccsendstats's main path on purpose: that path reads a
...
const PORT = +(process.env.CCSENDSTATS_PROXY_PORT || 8789);
const FORWARD = process.env.CCSENDSTATS_PROXY_FORWARD_URL || 'https://api.anthropic.com';
const LOG = process.env.CCSENDSTATS_PROXY_LOG || path.join(os.homedir(), '.ccsendstats-proxy', 'requests.jsonl');
    ...
    const upstream = lib.request({
      ...
      up.on('end', () => {
        ...
        if (info) {
          ...
          logRecord(rec);
          const u = usage || {};
          console.error(`[ccsendstats proxy] ${info.model}  sys=${(info.system_chars / 1000).toFixed(1)}k chars  msgs=${info.messages}  tools=${info.tools_offered}  ` +
...
function report() {
  ...
  console.log(`=== ccsendstats proxy-report (${lines.length} requests, ${withUsage} with usage) ===`);
...
function serve() {
  ...
  server.listen(PORT, '127.0.0.1', () => {
    console.error(`[ccsendstats proxy] observing on http://localhost:${PORT} → ${FORWARD}`);
    console.error(`[ccsendstats proxy] usage: ANTHROPIC_BASE_URL=http://localhost:${PORT} claude`);
    console.error('[ccsendstats proxy] passthrough is byte-for-byte; auth headers are never logged');
  });
  return server;
}
